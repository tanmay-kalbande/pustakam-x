...
Return ONLY valid JSON:
{
  ...
  buildModulePrompt(
    ...
    const summarize = (content: string): string => {
      ...
      if (words.length <= contextSummaryWords) return content;
    ...
    const contextSummary = !isFirstModule && previousModules.length > 0
      ? `\n\nWHAT WE ALREADY SMASHED (${contextMode === 'summary' ? `Summarized • ~${contextSummaryWords} words/module` : 'Full Context So Far'}):\n${previousModules
        .map((module, index) =>
