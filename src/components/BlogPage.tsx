...
import { elevationTokens } from '../styles/designTokens';
...
const BarChartVisual: React.FC<{ data: { label: string; value: number; color: string }[]; title: string }> = ({ data, title }) => {
    ...
    return (
        <div className="my-8 p-6 panel-primary elevation-1">
            <h4 className="text-sm font-semibold text-white/85 mb-5 uppercase tracking-wider flex items-center gap-2">
            ...
            <div className="relative mb-3">
                ...
                <div className="flex justify-between text-[9px] text-white/35 uppercase tracking-wider pt-1">
            ...
            <div className="space-y-3">
                {data.map((item, i) => (
                    <div key={i} className="flex items-center gap-3">
                        <span className="text-[11px] text-white/60 w-28 shrink-0 text-right font-medium">{item.label}</span>
                        <div className="flex-1 h-8 bg-white/[0.04] rounded-lg overflow-hidden relative border border-white/10">
                            <div className="absolute inset-y-0 left-1/2 w-px bg-white/10" />
                            <div
                                className="h-full rounded-lg transition-all duration-1000 ease-out flex items-center justify-end pr-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.25)]"
            ...
            <div className="mt-4 flex items-center justify-between text-[10px] text-white/40 uppercase tracking-wider">
                <span>Lower Preference</span>
                <span>Higher Preference</span>
            </div>
        </div>
    );
};
...
const StatGrid: React.FC<{ stats: { label: string; value: string; icon: React.ElementType; color: string }[] }> = ({ stats }) => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 my-8">
        {stats.map((stat, i) => (
            <div key={i} className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 text-center group hover:bg-white/[0.06] transition-all duration-300 elevation-1">
...
const ComparisonTable: React.FC<{ headers: string[]; rows: { feature: string; col1: string; col2: string }[]; colors: [string, string] }> = ({ headers, rows, colors }) => (
    <div className="my-8 panel-primary overflow-hidden elevation-1">
        <div className="grid grid-cols-3 bg-white/[0.05] border-b border-white/10">
            {headers.map((h, i) => (
                <div key={i} className={`px-4 py-3 text-[10px] font-semibold uppercase tracking-wider ${i === 0 ? 'text-white/40' : i === 1 ? colors[0] : colors[1]}`}>
                    {h}
                </div>
            ))}
        </div>
        {rows.map((row, i) => (
            <div key={i} className={`grid grid-cols-3 ${i % 2 === 0 ? 'bg-white/[0.02]' : 'bg-transparent'} border-b border-white/[0.04] last:border-0`}>
        ...
        {steps.map((step, i) => (
            <div key={i} className="flex gap-4 relative">
                <div className="flex flex-col items-center">
                    ...
                    <span className="text-[10px] text-white/40 mt-1 font-bold">{`0${i + 1}`.slice(-2)}</span>
...
const ProcessFlowDiagram: React.FC<{ steps: { title: string; desc: string; icon: React.ElementType; color: string }[] }> = ({ steps }) => (
    <div className="my-8 panel-primary p-4 md:p-6 elevation-1">
        <h4 className="text-sm font-semibold text-white/85 mb-4 uppercase tracking-wider">Generation Pipeline (Visual Map)</h4>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            {steps.map((step, index) => (
                <div key={step.title} className="relative">
                    <div className={`rounded-xl border p-4 h-full ${step.color} bg-black/20`}> 
                        <div className="flex items-center justify-between mb-2">
                            <step.icon size={16} />
                            <span className="text-[10px] font-bold text-white/70">Step {index + 1}</span>
                        </div>
                    ...
                    {index < steps.length - 1 && (
                        <div className="hidden md:flex absolute -right-2 top-1/2 -translate-y-1/2 text-white/35">
                            <ArrowRight size={14} />
                        </div>
                    )}
                </div>
...
const InfoCallout: React.FC<{ title: string; text: string; color?: string }> = ({ title, text, color = 'cyan' }) => (
    <div className={`my-6 p-5 rounded-2xl border ${
        color === 'emerald' ? 'bg-emerald-500/[0.06] border-emerald-500/15' :
            color === 'purple' ? 'bg-purple-500/[0.06] border-purple-500/15' :
...
const ensureMetaTag = (nameOrProperty: 'name' | 'property', key: string, content: string) => {
    let meta = document.head.querySelector(`meta[${nameOrProperty}="${key}"]`) as HTMLMetaElement | null;
    if (!meta) {
    ...
    },
    {
        ...
        content: (
            <>
                ...
                />
                <p className="text-[10px] meta-secondary -mt-4 mb-6 italic">* Chart shows % users choosing Street Mode. Remaining % chose Stellar.</p>
    ...
    },
    {
        ...
        content: (
            <>
                ...
                />
                <p className="text-[10px] meta-secondary -mt-4 mb-6 italic">* % of users who generate a second book, by first roadmap generation time.</p>
...
const BlogTopHeader: React.FC<BlogTopHeaderProps> = ({ title, backLabel, onBack, onClose, compact = false }) => (
    <header
        className="overlay-top-header sticky top-0 z-30"
        style={{ paddingTop: 'max(env(safe-area-inset-top), 0px)' }}
    >
        <div className={`mx-auto flex items-center justify-between px-4 md:px-6 py-3 ${compact ? 'max-w-4xl' : 'max-w-6xl'}`}>
            <button
                onClick={onBack}
                className="focus-ring flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-sm text-white/75 hover:bg-white/[0.06] hover:text-white transition-colors"
            >
                <ArrowLeft size={16} />
                <span>{backLabel}</span>
            </button>
            ...
            <button
                onClick={onClose}
                className="focus-ring rounded-lg p-2 text-white/75 hover:bg-white/[0.06] hover:text-white transition-colors"
...
const BlogPage: React.FC<BlogPageProps> = ({ onClose }) => {
    ...
    const articleSchema = useMemo(() => {
        ...
        return {
            ...
            keywords: `${selectedPost.category}, AI learning, personalized education, book generation`
        };
    }, [selectedPost]);

    useEffect(() => {
        if (selectedPost) {
            document.title = `${selectedPost.title} | Pustakam Blog`;
            ensureMetaTag('name', 'description', selectedPost.excerpt);
            ensureMetaTag('property', 'og:title', selectedPost.title);
            ensureMetaTag('property', 'og:description', selectedPost.excerpt);
            ensureMetaTag('property', 'og:type', 'article');
            ensureMetaTag('name', 'twitter:card', 'summary_large_image');
            ensureMetaTag('name', 'twitter:title', selectedPost.title);
            ensureMetaTag('name', 'twitter:description', selectedPost.excerpt);
        } else {
            document.title = 'Pustakam Blog | AI Learning, Product Engineering & Insights';
            ensureMetaTag('name', 'description', 'Read deep technical and product essays on AI book generation, adaptive learning design, and practical engineering lessons from building Pustakam.');
            ensureMetaTag('property', 'og:type', 'website');
            ensureMetaTag('property', 'og:title', 'Pustakam Blog');
            ensureMetaTag('property', 'og:description', 'Technical breakdowns, growth lessons, and product thinking behind Pustakam AI.');
        }
    }, [selectedPost]);
    ...
    useEffect(() => {
        const timeout = setTimeout(() => setIsCardsLoading(false), 360);
    ...
    if (selectedPost) {
        return (
            <div ref={scrollContainerRef} className="fixed inset-0 z-[100] bg-black overflow-auto overscroll-contain">
                ...
                {/* Article Content */}
                <article className="max-w-3xl mx-auto px-5 md:px-6 py-10 md:py-12 relative z-10 density-prose">
                    {/* Meta */}
                    <div className="mb-7">
                        ...
                        <h1 className="text-3xl md:text-[2.75rem] font-semibold text-white mb-3 leading-tight tracking-tight">
                            {selectedPost.title}
                        </h1>
                        <p className="text-base md:text-lg text-white/60 mb-5 font-normal leading-relaxed">{selectedPost.subtitle}</p>
                        <div className="flex flex-wrap items-center gap-4 text-xs meta-secondary">
                    ...
                    {/* Blog Body */}
                    <div className="prose-pustakam [&>p]:text-[16px] [&>p]:leading-8 [&>p]:text-white/78 [&>h3]:text-[1.55rem] [&>h3]:font-semibold [&>h3]:mt-10 [&>h3]:mb-4 [&>ul]:text-white/75 [&>ul]:leading-8">
                    ...
                    {/* Author Card */}
                    <div className="mt-14 rounded-2xl border border-white/10 bg-white/[0.02] p-5">
                    ...
                    <div className="mt-9 grid grid-cols-1 md:grid-cols-2 gap-3">
                        {BLOG_POSTS.filter(p => p.id !== selectedPost.id).slice(0, 2).map(post => (
                            <button
                                ...
                                className="focus-ring text-left p-4 panel-secondary hover:bg-white/[0.05] hover:border-white/10 transition-all group"
                            >
    ...
    return (
        <div ref={scrollContainerRef} className="fixed inset-0 z-[100] bg-black overflow-auto overscroll-contain">
            ...
            <main className="max-w-6xl mx-auto px-5 md:px-8 py-10 md:py-12 relative z-10">
                {/* Hero Section */}
                ...
                {isCardsLoading ? (
                    <div className="rounded-3xl border border-white/10 p-8 md:p-12">
                        <div className="h-4 w-32 skeleton-shimmer rounded mb-5" />
                        <div className="h-10 md:h-14 skeleton-shimmer rounded mb-4" />
                        <div className="h-4 skeleton-shimmer rounded mb-2" />
                        <div className="h-4 w-10/12 skeleton-shimmer rounded mb-7" />
                    ...
                    return (
                        <button
                            onClick={() => openPost(BLOG_POSTS[0].id)}
                            className="focus-ring w-full text-left mb-8 group"
                        >
                            <div className={`relative rounded-3xl bg-gradient-to-br from-cyan-500/[0.06] to-purple-500/[0.04] border border-white/10 p-8 md:p-12 hover:border-white/15 transition-all duration-500 overflow-hidden ${elevationTokens.medium}`}>
                                {/* Glow accent */}
                                ...
                                <div className="relative z-10">
                                    ...
                                    <h2 className="text-3xl md:text-5xl font-semibold text-white mb-3 group-hover:text-cyan-50 transition-colors tracking-tight leading-tight">
                                        {BLOG_POSTS[0].title}
                                    </h2>
                                    <p className="text-base text-white/60 max-w-3xl leading-relaxed mb-6">{BLOG_POSTS[0].excerpt}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {(isCardsLoading ? Array.from({ length: 4 }).map((_, idx) => ({ id: `s-${idx}` })) : BLOG_POSTS.slice(1)).map((post: any) => (
                        isCardsLoading ? (
                        <div key={post.id} className="h-full panel-primary p-7">
                            <div className="h-4 w-20 skeleton-shimmer rounded mb-4" />
                            <div className="h-6 skeleton-shimmer rounded mb-2" />
                            <div className="h-6 w-5/6 skeleton-shimmer rounded mb-4" />
                            <div className="h-4 skeleton-shimmer rounded mb-2" />
                            <div className="h-4 w-4/5 skeleton-shimmer rounded mb-6" />
                        ...
                        >
                            <div className={`h-full panel-primary ${elevationTokens.low} p-7 hover:bg-white/[0.04] hover:border-white/12 transition-all duration-400 flex flex-col`}>
                                <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-xl md:text-2xl font-semibold text-white/90 mb-2 group-hover:text-white transition-colors leading-snug tracking-tight">
                                    {post.title}
                                </h3>
                                <p className="text-sm text-white/55 leading-relaxed mb-5 flex-1">{post.excerpt}</p>
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] text-white/20 flex items-center gap-1.5">
                                        <Clock size={10} />{post.readTime}
                                    </span>
                                    <span className="flex items-center gap-1 text-[10px] meta-secondary font-semibold uppercase tracking-wider group-hover:text-white/60 transition-colors">
