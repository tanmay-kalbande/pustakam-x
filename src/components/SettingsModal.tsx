...
import { X, Shield, Database, Download, Upload, Trash2, Key, Settings, ExternalLink, Eye, EyeOff, User, Zap, Globe, Cpu, BookOpen, AlertTriangle, BookMarked, ChevronRight, Crown, Sparkles, Calendar, Sun, Moon, Info } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { APISettings } from '../types';
import { storageUtils } from '../utils/storage';
import { DisclaimerPage } from './DisclaimerPage';
import { MotionPolicy, useMotionPolicy } from '../hooks/useMotionPolicy';
import { ContentDensity, useContentDensity } from '../hooks/useContentDensity';
...
export function SettingsModal({ isOpen, onClose, settings, onSaveSettings, theme, onToggleTheme, onOpenAPIDocs, onOpenUsageGuide, onOpenCompliance, showAlertDialog }: SettingsModalProps) {
  ...
  const { motionPolicy, setMotionPolicy } = useMotionPolicy();
  ...
  const apiConfigs = [
    ...
    {
      id: 'xaiApiKey' as keyof APISettings,
      name: 'xAI',
      url: 'https://console.x.ai/',
      description: 'For Grok 4.1 and Grok 4 Fast models'
    },
    {
      id: 'groqApiKey' as keyof APISettings,
      name: 'Groq',
      url: 'https://console.groq.com/keys',
      description: 'For Llama 4 Maverick and other models'
    },
    {
      id: 'openRouterApiKey' as keyof APISettings,
      name: 'OpenRouter',
      url: 'https://openrouter.ai/keys',
      description: 'For Trinity models and other various providers'
    },
    {
      id: 'cohereApiKey' as keyof APISettings,
      name: 'Cohere',
                ...
                <div className="space-y-8">
                  ...
                  <section className="space-y-6">
                    ...
                    <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-white/[0.05]">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Motion Policy</label>
                      <div className="grid grid-cols-3 gap-2 p-1 bg-gray-100 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                        {(['normal', 'reduced', 'off'] as MotionPolicy[]).map((policy) => (
                          <button
                            key={policy}
                            onClick={() => setMotionPolicy(policy)}
                            className={`focus-ring flex items-center justify-center gap-2.5 py-3 rounded-lg transition-all duration-200 ${motionPolicy === policy
                              ? (theme === 'light' ? 'bg-white text-gray-900 shadow-md ring-1 ring-black/5' : 'bg-cyan-500/20 text-cyan-300 shadow-md ring-1 ring-white/10')
                              : 'text-gray-500 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}
                          >
                            <span className="font-bold text-sm capitalize">{policy}</span>
                          </button>
                        ))}
                      </div>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 italic">Choose global animation intensity: full motion, reduced motion, or fully static UI.</p>
                    </div>



                    <div className="space-y-4 pt-6 border-t border-gray-100 dark:border-white/[0.05]">
                      <label className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Content Density</label>
                      <div className="grid grid-cols-2 gap-2 p-1 bg-gray-100 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                        {(['comfortable', 'compact'] as ContentDensity[]).map((option) => (
                          <button
                            key={option}
                            onClick={() => setDensity(option)}
                            className={`focus-ring flex items-center justify-center gap-2.5 py-3 rounded-lg transition-all duration-200 ${density === option
                              ? (theme === 'light' ? 'bg-white text-gray-900 shadow-md ring-1 ring-black/5' : 'bg-cyan-500/20 text-cyan-300 shadow-md ring-1 ring-white/10')
                              : 'text-gray-500 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}
                          >
                            <span className="font-bold text-sm capitalize">{option}</span>
                          </button>
                        ))}
                      </div>
                      <p className="text-[10px] text-gray-400 dark:text-gray-500 italic">Controls prose spacing on Blog and API Docs (comfortable for reading, compact for scanning).</p>
                    </div>
                ...
                <div className="space-y-10">
                  <div className="flex items-start gap-6">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Knowledge Synthesis v1.3</p>
