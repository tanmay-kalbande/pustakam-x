...
function App() {
  ...
  const getAlternativeModels = () => {
    ...
    if (settings.xaiApiKey && settings.selectedProvider !== 'xai')
      alternatives.push({ provider: 'xai', model: 'grok-beta', name: 'xAI Grok' });
    ...
    if (settings.cerebrasApiKey && settings.selectedProvider !== 'cerebras')
      alternatives.push({ provider: 'cerebras', model: 'llama3.1-70b', name: 'Cerebras Llama 3.1 70B' });
    ...
    <div className="app-container">
      ...
      {isLoadingScreenVisible && (
        <LoadingScreen
          ...
          message={isAuthTransitioning ? "Entering your workspace..." : "Initializing Pustakam..."}
        />
      )}
    </div>
  );
